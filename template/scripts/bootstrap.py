...
HARNESS_FILES: dict[str, list[str]] = {
    "claude":   [".claude/settings.json", ".claude/sdlc-contract.md", "CLAUDE.template.md"],
    "cursor":   [".cursor-plugin/plugin.json", ".cursor-plugin/hooks/hooks-cursor.json",
                 ".cursor/rules/sdlc-contract.mdc"],
    "codex":    [".codex/INSTALL.md", "AGENTS.md"],
    "gemini":   ["gemini-extension.json", "GEMINI.md"],
    "copilot":  [".claude-plugin/plugin.json", "AGENTS.md"],
    "opencode": [".opencode/INSTALL.md", ".opencode/plugins/sdlc-contract-loader.js", "AGENTS.md"],
}

# Always-copy (no harness-scoping): phase tree + skill + hooks + scripts + config + docs + schemas.
ALWAYS_COPY_DIRS = [
    "docs/sdlc",
    "skill", "hooks", "scripts", "config", "docs", "schemas",
]
...
def main() -> int:
    ...
    # re-pins if they later amend the contract.
