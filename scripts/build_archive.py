...
# The kit's own SDLC artifacts (docs/sdlc/01_requirement..docs/sdlc/06_maintenance at repo root) are
# NOT shipped — they are our requirements/design/CRs, not adopter-facing.
# Adopters get blank scaffolds via template/0X_*/.
DIST_INCLUDES = [
...
EXCLUDES = {".DS_Store", "__pycache__", "*.pyc", ".metrics", ".git",
            "compare_skill_versions.py"}  # dev-only tool, hardcoded maintainer path
...
def main(argv: list[str]) -> int:
    ...
    # Exclude the kit's own phase dogfood from dist (adopters get blank
    # scaffolds via template/docs/sdlc/). This mirrors the pre-reorg
    # behavior where 01_..06_/ at repo root were not in DIST_INCLUDES.
    live_phases = dist / "docs" / "sdlc"
    if live_phases.exists():
        shutil.rmtree(live_phases)
